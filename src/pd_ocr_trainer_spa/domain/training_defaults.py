...
404s and the SPA falls back to the seed (the ``pd-ocr-training`` config-model
...
# Seed defaults — verbatim from spec 04 §3.2 (the pd-ocr-training
...
def seed_defaults(task: TaskEnum) -> dict[str, object]:
    ...
    whose config models are still a future ``pd-ocr-training`` addition.
    """

...
Invoked as::
    ...
    python -m pdomain_ocr_trainer_spa.worker.evaluate --run-dir runs/<id>
...
``pdomain_ocr_training`` now exposes ``IEvalRunner`` + ``LocalEvalRunner`` and the
...
* ``evaluate_recognition_impl`` hard-codes ``slices=[]`` (see
  ``pdomain_ocr_training/_eval_backend.py`` — "Out of scope (issue #3 baseline)").
...
The pdomain-book-tools ``GlyphAnnotations`` data model (the other M13 pre-condition)
has landed, but until ``pdomain_ocr_training`` accepts annotations as eval input and
...
from pdomain_ocr_trainer_spa.core.models import EvalMetrics, EvalResult
...
def _build_runner() -> EvalRunner:
    ...
    pdomain-ocr-training exposes ``LocalEvalRunner`` (``IEvalRunner``), but wiring it
    ...
    """
    raise RuntimeError(
        "Real model evaluation is not wired up yet. pdomain-ocr-training exposes "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="pdomain_ocr_trainer_spa.worker.evaluate")

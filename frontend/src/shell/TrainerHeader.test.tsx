...
vi.mock("@pdomain/pdomain-ui/shell", () => ({
  JobsPill: ({ activeJobs }: { activeJobs: unknown[] }) =>
    createElement(
      "button",
      { "data-testid": "jobs-pill", "aria-label": "Jobs" },
      `${activeJobs.length} jobs`,
    ),
  ShortcutsHelpButton: () =>
    createElement("button", { "aria-label": "Keyboard shortcuts" }, "?"),
  SettingsSlot: () =>
    createElement("button", { "aria-label": "Settings" }, "⚙"),
...
describe("TrainerHeader", () => {
  it("renders header-bar testid", () => {
    render(
      createElement(TrainerHeader, { activeJobs: [], appVersion: "0.1.0" }),
    );
  ...
  it("renders header-app-version testid", () => {
    ...
    expect(screen.getByTestId("header-app-version").textContent).toContain(
      "0.1.0",
    );
  });
